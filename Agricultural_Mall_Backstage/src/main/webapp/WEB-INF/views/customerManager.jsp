...
<head>
    ...
    <script>
        ...
        $(function () {
            ...
            //检测修改用户信息的modal 框隐藏的时候，将校验的数据清空
            ...
            <table class="table table-bordered table-hover" style='text-align: center;'>
                ...
                <c:forEach items="${pageInfo.list}" var="order">
                    <tr>
                        ...
                        <td>${order.address}</td>
                        <td>
                            <c:if test="${order.isValid == 1}">有效</c:if>
                            <c:if test="${order.isValid == 0}">无效</c:if>
                        </td>
                        <td class="text-center">
                            <input type="button" class="btn btn-warning btn-sm doModify"
                                   onclick="showCustomer(${order.id})" value="修改">
                            <c:if test="${order.isValid == 0}">
                                <input type="button" class="btn btn-success btn-sm doDisable"
                                       onclick="modifyCustomerStatus(${order.id},this)" value="启用">
                            </c:if>
                            <c:if test="${order.isValid == 1}">
                                <input type="button" class="btn btn-danger btn-sm doDisable"
                                       onclick="modifyCustomerStatus(${order.id},this)" value="禁用">
                            </c:if>
                        </td>
                    </tr>
                </c:forEach>
