...
import com.gold.mall.common.utils.*;
...
@Slf4j
@Controller
@RequestMapping("/reception/wxPay")
public class WxPayController {
    ...
    @ResponseBody
    public ResponseResult goWxPay(String orderNumber, HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (WxPayUtils.isCorrectSign(sortedMap, WxPayConfig.wxpay_key)) {
            ...
            if ("SUCCESS".equals(sortedMap.get("return_code"))) {
                ...
                if ("SUCCESS".equals(sortedMap.get("result_code"))) {
                    ...
                    //通知微信，异步确认成功，必须响应，要不然微信后台会一直通知后台处理支付，超过8次则认为订单交易失败
    ...
    public ModelAndView ShowPayStatus(String orderNumber, HttpSession session) {
        ...
        mv.addObject("productName", "A-Mall");
