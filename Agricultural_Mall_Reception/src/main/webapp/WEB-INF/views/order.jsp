...
<head>
    ...
    <script>
        ...
        $(function () {
                ...
                fields: {
                    ...
                    receiverProvince: {
                        validators: {
                            remote: {
                                type: 'post',
                                url: '${pageContext.request.contextPath}/reception/address/checkProvince'
                            }
                        }
                    },
                    receiverCity: {
                        validators: {
                            remote: {
                                type: 'post',
                                url: '${pageContext.request.contextPath}/reception/address/checkCity'
                            }
                        }
                    },
                    receiverDistrict: {
                        validators: {
                            remote: {
                                type: 'post',
                                url: '${pageContext.request.contextPath}/reception/address/checkDistrict'
                            }
                        }
                    },
                    addressDetail: {
            ...
            //修改地址表单校验
            $('#frmModifyAddress').bootstrapValidator({
                feedbackIcons: {
                    valid: 'glyphicon glyphicon-ok',
                    invalid: 'glyphicon glyphicon-remove',
                    validating: 'glyphicon glyphicon-refresh'
            ...
            $('#myAddressAddModal').on('hide.bs.modal', function () {
                $('#frmAddAddress').bootstrapValidator('resetForm');
                $('#frmAddAddress')[0].reset();
            });

            //确认之后,清空原来模态框中校验的数据
            $('#myAddressModifyModal').on('hide.bs.modal', function () {
                $('#frmModifyAddress').bootstrapValidator('resetForm');
                $('#frmModifyAddress')[0].reset();
            });
            //当是否立即支付提示框消息后，即客户不跳转到支付页面，则跳转到订单详情页
            $('#buildOrderModal').on('hide.bs.modal', function () {
                location.replace('${pageContext.request.contextPath}/reception/order/showOrderDetails');
            });
        });

        //设置固定在底部的footer 可以自适应高度
        ...
        function showModifyAddress() {
            ...
            if (spid == '') {
                ...
                $.post(
                    '${pageContext.request.contextPath}/reception/address/showOneAddress',
                    {'addressId': $('#modifyId').val()},
                    function (result) {
                        if (result.status == 1) {
                            //成功,则设置值显示
                            ...
                            //使用area.js 三级地址联动，无法实现模态框中设置回显值
                            // $('#s_province').val(result.data.receiverProvince);
                            // change(1);
                            // $('#s_city').val(result.data.receiverCity);
                            // change(2);
        ...
        function modifyAddress() {
            $('#frmModifyAddress').data('bootstrapValidator').validate();
            var flag = $('#frmModifyAddress').data('bootstrapValidator').isValid();
            if (flag) {
                $.post(
                    '${pageContext.request.contextPath}/reception/address/modifyAddress',
                    $('#frmModifyAddress').serialize(),
                    function (result) {
                        if (result.status == 1) {
                            //成功，重新加载页面
                            layer.msg(result.message, {
        ...
        function addAddress() {
            //先进行表单校验
            $('#frmAddAddress').data('bootstrapValidator').validate();
            var flag = $('#frmAddAddress').data('bootstrapValidator').isValid();
            if (flag) {
                $.post(
                    '${pageContext.request.contextPath}/reception/address/saveAddress',
                    $('#frmAddAddress').serialize(),
                    function (result) {
                        if (result.status == 1) {
                            //地址新增成功
                            layer.msg(result.message, {
        ...
        function removeAddress() {
            $.post(
                '${pageContext.request.contextPath}/reception/address/removeAddress',
        ...
        function commitOrders() {
            var addressId = $('#orderAddress').val();
            var status = '${orderList.get(0).status}';
            <%--if (status == 2) {--%>
            <%--//表示走直接购买的流程--%>
            <%--$.post(--%>
            <%--'${pageContext.request.contextPath}/reception/order/redirectAddOrder',--%>
            <%--{'addressId': addressId},--%>
            <%--function (result) {--%>
            <%--if (result.status == 1) {--%>
            <%--//创建订单成功--%>
            ...
            <%--} else {--%>
            $.post(
                '${pageContext.request.contextPath}/reception/order/addOrder',
                {'addressId': addressId, 'status': status},
                function (result) {
                    if (result.status == 1) {
                        //创建订单成功
                        $('#orderNo').text(result.message);
        ...
        function backCart() {
            ...
            if (status == 2) {
                //把这个购物车清空
                $.post(
                    '${pageContext.request.contextPath}/reception/cart/removeOneProduct',
                    {'cartId': cartId},
                    function (result) {
                        if (result.status == 1) {
                        } else {
        ...
        function payOrderFromOrder() {
            ...
            location.href = '${pageContext.request.contextPath}/reception/order/showPayOrders?orderNumber=' + orderNumber;
        }
                        ...
                        <c:forEach items="${addressList}" var="address">
                            <li class="col-sm-3" style="margin-top: 10px">
                                ...
                                <div class="mydelSpan" style="padding-top:8px">
                                    <p>${address.receiverProvince}${address.receiverCity}${address.receiverDistrict}</p>
                                    <p>${address.addressDetail}</p>
                                    <p>${address.receiverMobile}</p>
                                    <input type="hidden" name="addressId" value="${address.id}">
                                    <span class="btn btn-xs"
                                          style="color:#8c8c8c;font-size: 10px;margin-bottom: 5px;padding: 0"
                                          onclick="showDelModal(${address.id})">删除</span>
                                </div>
                            </li>
                            <input type="hidden" name="addressId" value="${address.id}">
                        </c:forEach>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body" style="overflow: auto">
                <form class="form-horizontal col-sm-10" id="frmModifyAddress">
            ...
            <div class="modal-footer">
                <input type="hidden" name="addressId" id="modifyId">
                <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                <button type="button" class="btn btn-primary" onclick="modifyAddress()">确认修改</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="myAddressAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form class="form-horizontal" style="width: 85%" id="frmAddAddress">
                <div class="modal-body" style="overflow: auto">
                    <div class="form-group">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                    <button type="button" id="addAddressBtn" class="btn btn-primary" onclick="addAddress()">确认添加
                    </button>
                </div>
            </form>
        </div>
    ...
    <div class="modal-dialog modal-sm">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-primary updateProType" onclick="removeAddress()" data-dismiss="modal">确认
                </button>
