...
<head>
    ...
    <script>
        ...
        function addToCart(id) {
            $.post(
                '${pageContext.request.contextPath}/reception/cart/addToCart',
                {'id': id, 'productNum': $('#text_box').val()},
                function (result) {
                    if (result.status == 1) {
                        //商品成功添加购物车
        ...
        function buy() {
            ...
            $.post(
                '${pageContext.request.contextPath}/reception/order/redirectBuyToCart',
                {'count': count, 'productId': productId},
                function (result) {
                    if (result.status == 1) {
                        //表示商品成功放入购物车中
                        console.log(result);
                        location.replace('${pageContext.request.contextPath}/reception/order/redirectConfirmOrder?cartId=' + result.data);
                    } else {
                        layer.msg("请先登录！", {
                            time: 1500,
                            skin: 'errorMsg'
