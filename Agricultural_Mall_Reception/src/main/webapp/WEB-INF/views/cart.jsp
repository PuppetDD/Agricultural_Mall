...
<head>
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/mallStyle.css"/>
    ...
    <c:choose>
        <c:when test="${empty cartList}">
            <h3 class="text-success" style="text-align: center;margin-top: 180px">
                购物车啥也没有哦, <a class="btn-link"
                             href="${pageContext.request.contextPath}/reception/product/searchAllProducts">前往购物吧~</a>
            </h3>
        </c:when>
        <%--如果购物车为空则让它看到的是空页面--%>
        <c:otherwise>
            <table class="table table-hover table-striped table-bordered table-condensed text-center">
                ...
                <tr>
                    <td colspan="8" align="right" style="height: 60px">
                        ...
                        <button class="btn btn-warning margin-right-15" type="button" onclick="goToShopping()"> 继续购物
                        </button>
...
<script>
    ...
    function inputModifyProductNum(cartId, num) {
        ...
        $.post(
            '${pageContext.request.contextPath}/reception/cart/inputModifyProductNum',
    ...
    function removeAllProduct() {
        $.post(
            '${pageContext.request.contextPath}/reception/cart/clearAllProductFromCart',
            function (result) {
                if (result.status == 1) {
                    //表示购物车清空成功
                    // layer.msg(result.message,{
    ...
    function removeMoreProduct() {
        if ($('#cartIds').val().length > 0) {
            $.post(
                '${pageContext.request.contextPath}/reception/cart/removeMoreProductFromCart',
                {'cartIds': $('#cartIds').val()},
                function (result) {
                    if (result.status == 1) {
                        //成功则刷新页面
    ...
    function settleAccounts() {
        ...
        if (selects.length == 0) {
            ...
            $.post(
                '${pageContext.request.contextPath}/reception/cart/addTempOrderItem',
                {'count': count, 'price': price, 'orderCartIds': orderCartIds},
                function (result) {
                    if (result.status == 1) {
                        //成功，则转到确认订单页面
                        location.href = '${pageContext.request.contextPath}/reception/order/confirmOrder';
                    }
                }
            );
        }
