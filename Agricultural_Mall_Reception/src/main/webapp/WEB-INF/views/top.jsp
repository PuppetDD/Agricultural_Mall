...
<script id="welcome" type="text/html">
    <li class="userName">
        <span id="customerName" class="text text-success">{{name}} !</span>
    </li>
    <li class="dropdown">
...
<script>
    ...
    $(function () {
        ...
        //找到所有的li标签，对其 a 标签的链接地址进行判断，
        //如何和当前浏览器的地址一致，就认为是当前应该激活的菜单，添加active类，否则就取消
            ...
            fields: {
                ...
                verifyCode: {
                    validators: {
                        ...
                        remote: {
                            type: 'post',
                            url: '${pageContext.request.contextPath}/reception/sms/CheckSmsCode'
                        }
                    }
                }
            }
            ...
            fields: {
                password: {
                    validators: {
                        ...
                        remote: {
                            type: 'post',
                            url: '${pageContext.request.contextPath}/reception/customer/checkPassword'
                        }
                    }
                },
                newpassword: {
            ...
            fields: {
                ...
                loginName: {
                    validators: {
                        ...
                        remote: {
                            type: 'post',
                            url: '${pageContext.request.contextPath}/reception/customer/checkLoginName'
                        }
                    }
                },
                password: {
        ...
        //模态框隐藏时候，清除校验信息
        $('#registerModal').on('hide.bs.modal', function () {//监听modal消失
            //清除校验信息
            $('#frmRegister').bootstrapValidator('resetForm');
            //表单中填写的信息也清除
            $('#frmRegister')[0].reset();
        });
    ...
    function loginByAccount() {
        ...
        if (flag) {
            //校验通过发送ajax请求
            $.post(
                '${pageContext.request.contextPath}/reception/customer/loginByAccount',
                /*表单序列化直接获取值传给后端*/
                $('#frmLoginByAccount').serialize(),
                function (result) {
                    if (result.status == 1) {
                        //这里刷新整个页面刷新页面，解决点击购物车，如果没登录时候，登录后没能及时刷新问题（登录成功购物车页面数据不能立即加载显示，以及客户的头像不能及时更新）；
                        //后续需要考虑有没有局部刷新的同时也实现页面数据的刷新
                        location.href = '${pageContext.request.contextPath}/reception/product/searchAllProducts';
                        /*使用局部刷新*/
                        // 隐藏模态框
                        $('#loginModal').modal('hide');
                        // 局部刷新实现：加载模板,传入数据到模板中去渲染
    ...
    function logout() {
        $.post(
            '${pageContext.request.contextPath}/reception/customer/logout',
            function (result) {
                if (result.status == 1) {
                    location.href = '${pageContext.request.contextPath}/reception/product/searchAllProducts';
                }
    ...
    function sendVerifyCode(btn) {
        $.post(
            '${pageContext.request.contextPath}/reception/sms/sendVerifyCode',
            {'phone': $('#phone').val()},
            function (result) {
                if (result.status == 1) {
                    //定义倒计时
    ...
    function loginBySms() {
        ...
        if (flag) {
            $.post(
                '${pageContext.request.contextPath}/reception/customer/loginBySms',
    ...
    function CustomerRegister() {
        ...
        $('#frmRegister').data('bootstrapValidator').validate();
        var valid = $('#frmRegister').data('bootstrapValidator').isValid();
        if (valid) {
            $.post(
                '${pageContext.request.contextPath}/reception/customer/register',
                ...
                function (result) {
                    if (result.status == 1) {
                        ...
                        // 加载模板,传入数据到模板中去渲染,这块局部刷新，头像显示会有问题
                        // var content = template('welcome', result.data);
                        // $('#navbarInfo').html(content);
    ...
    function modifyPassword() {
        ...
        if (flag) {
            $.post(
                '${pageContext.request.contextPath}/reception/customer/modifyPassword',
    ...
    function showShopCarts() {
        location.href = '${pageContext.request.contextPath}/reception/cart/myCarts';
    }

    /*展示用户所有订单列表*/
    function showOrderDetails() {
        location.href = '${pageContext.request.contextPath}/reception/order/myOrders';
    }

    /*展示用户个人中心*/
    function showCustomerInfo() {
        location.href = '${pageContext.request.contextPath}/reception/customer/customerCenter';
    }
    ...
    <div class="container">
        <div class="navbar-header">
            <a class="navbar-brand logo-style" target="_blank"
               href="${pageContext.request.contextPath}/reception/product/searchAllProducts">
                <img class="brand-img" src="${pageContext.request.contextPath}/images/logo.png" alt="logo"
                     height="66">
            </a>
        </div>
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            <ul class="nav navbar-nav">
                <%--通过jsp include 传值的方式设置active--%>
                <li class="${param.num == 1 ? 'active' : ''}">
                    <a href="${pageContext.request.contextPath}/reception/product/searchAllProducts">商城主页</a>
                </li>
                <li class="${param.num == 2 ? 'active' : ''}" style="cursor: pointer;">
                    <%--<a id="shopCart" onclick="showShopCarts()"  href="${pageContext.request.contextPath}/reception/cart/myCarts">--%>
                    <a id="shopCart"
            ...
            <ul class="nav navbar-nav navbar-right" id="navbarInfo">
                <%--使用c:choose 判断 session中是否存有用户的信息--%>
                <c:choose>
                    <%--如果session中用户为空则走 when 中--%>
                    ...
                    <c:otherwise>
                        <li class="userName">
                            <span class="text text-success">${sessionScope.get("customer").name}</span>
                        </li>
                        <li class="dropdown">
                            <a href="#" class="dropdown-toggle user-active" data-toggle="dropdown" role="button">
        ...
        <div class="modal-content" style="width: 80%">
            ...
            <form id="frmRegister" class="form-horizontal">
                ...
                <div class="modal-footer" align="center">
                    ...
                    <button type="button" class="btn btn-warning" onclick="CustomerRegister()">注&nbsp;&nbsp;册</button>
                </div>
            </form>
        </div>
    </div>
